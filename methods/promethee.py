...
class Promethee:
    def __init__(self, decision_matrix:  DecisionMatrix, weights: list):
        ...
        self.performance_index_matrix = np.zeros((self.decision_matrix.alt_count, self.decision_matrix.alt_count))
            ...
            for j in range(self.decision_matrix.alt_count):
                if i != j:
                    for k in range(self.decision_matrix.crit_count):
                        diff = self.decision_matrix.normalized_matrix[i][k] - self.decision_matrix.normalized_matrix[j][k]
                        if diff > 0:
